...
class Trainer:
    def __init__(self, estimator, task='regression', metric=None, cv=None,
                 cv_args=None, use_early_stopping=False, verbose=False,
                 save=False, save_path=None):
        self.estimator = estimator
        self.metric = metric
        self.cv = cv
        self.groups = cv_args.get('groups') if cv_args else None
    ...
    def fit(self, X, y, fit_args=None):
        n = len(y)
        self.oof_preds = np.full(n, np.nan)
        groups = self.groups.values if hasattr(self.groups, 'values') else self.groups

        for fold, (tr_idx, va_idx) in enumerate(self.cv.split(X, y, groups)):
            ...
            self.oof_preds[va_idx] = pred_va
            score = self.metric(y_va, pred_va)
            self.fold_scores.append(score)
            if self.verbose:
                print(f"  Fold {fold+1} RMSE={score:.4f}")

        self.overall_score = self.metric(y, self.oof_preds)
        if self.verbose:
            print(f"  >>> OOF Overall RMSE={self.overall_score:.4f}")
    ...
    def predict(self, X_test):
        if len(self.models) > 1:
            preds = np.mean([m.predict(X_test) for m in self.models], axis=0)
        else:
...
class CFG:
    ...
    working_path = Path("/kaggle/working/artifacts")
...
class DenseANCCImputer:
    ...
    def impute(self, xy_q, self_wid=None, k=20, nfetch=5000):
...
def run_pf_ancc(hw, tw_tvt, tw_gr, N=600):
    ...
    pts, std = _pf_ancc(ev['MD'].values.astype(np.float64), ev['Z'].values.astype(np.float64),
                        ...
                        gs, ls, ir, N, 0.998, 0.002, 0.005, 0.3, 0.1, 0.001, 0.5)
    return pts.astype(np.float32), std.astype(np.float32)

def run_pf_z(hw, tw_tvt, tw_gr, N=500):
    gs = _gr_sig(hw, tw_tvt, tw_gr)
...
train_df.to_csv(CFG.working_path / "train_features.csv", index=False)
print(f"训练集形状: {train_df.shape}")

print("\n" + "="*50)
print("Step 2: 构建测试集特征...")
print("="*50)
...
test_df.to_csv(CFG.working_path / "test_features.csv", index=False)
...
print(f"特征数: {len(features)}, 训练样本: {len(X)}, 测试样本: {len(X_test)}")
...
lgb_params = [
    dict(boosting_type="gbdt", num_leaves=255, min_child_samples=15,
         ...
         learning_rate=0.03, n_estimators=5000, seed=123),
    dict(n_jobs=-1, verbose=-1, reg_alpha=10.788188919840913,
         subsample=0.47437582748953966, num_leaves=64,
         reg_lambda=95.75401894533888, n_estimators=10000,
...
cb_params = [
    dict(iterations=8000, depth=7, l2_leaf_reg=2.0,
         ...
         learning_rate=0.02, random_seed=7),
    dict(iterations=8000, depth=7, l2_leaf_reg=2.0,
         min_data_in_leaf=15, border_count=254,
         loss_function="RMSE", task_type="GPU", devices="0",
         od_type="Iter", od_wait=300, verbose=0,
         learning_rate=0.03, random_seed=123),
]

ridge_params = {"random_state": 42, "alpha": 1.6602834637650032,
...
for i, params in enumerate(lgb_params):
    save_path = CFG.working_path / f"lightgbm-{i+1}"
    model_file = save_path / "trainer.pkl"
    if model_file.exists():
        print(f"加载已训练的 lightgbm-{i+1}...")
        trainer = joblib.load(model_file)
    else:
        print(f"训练 lightgbm-{i+1}...")
        trainer = Trainer(
    ...
    print(f"lightgbm-{i+1} OOF RMSE: {trainer.overall_score:.4f}")

print("\n" + "="*50)
print("Step 4: 训练 CatBoost 模型...")
print("="*50)
for i, params in enumerate(cb_params):
    save_path = CFG.working_path / f"catboost-{i+1}"
    model_file = save_path / "trainer.pkl"
    if model_file.exists():
        print(f"加载已训练的 catboost-{i+1}...")
        trainer = joblib.load(model_file)
    else:
        print(f"训练 catboost-{i+1}...")
        trainer = Trainer(
    ...
    print(f"catboost-{i+1} OOF RMSE: {trainer.overall_score:.4f}")

print("\n" + "="*50)
print("Step 5: Ridge 元模型融合...")
print("="*50)
...
ridge_test = ridge_trainer.predict(test_preds_df)
overall_scores["ridge"] = ridge_trainer.overall_score
fold_scores["ridge"] = ridge_trainer.fold_scores
print(f"Ridge OOF RMSE: {ridge_trainer.overall_score:.4f}")
...
d = apply_pp(train_df, ridge_oof, pf_oof, **pp_params)
ridge_pp_score = root_mean_squared_error(ytrue, base + d)
print(f"Ridge + 后处理 OOF RMSE: {ridge_pp_score:.4f}")

print("\n" + "="*50)
print("Step 7: 生成 sub_1...")
print("="*50)
test_df2 = test_df.copy()
pf_test = test_df2['pf_ancc'].values - test_df2['last_known_tvt'].values
test_df2['pred'] = test_df2['last_known_tvt'].values + apply_pp(
    test_df2, ridge_test, pf_test, **pp_params
)
...
print(f"sub_1 行数: {len(sub_1)}")

print("\n" + "="*50)
print("Step 8: 生成 sub_2（粒子滤波 + 波束搜索 + 物理模型）...")
print("="*50)
...
for i, wid in enumerate(test_wells):
    print(f'处理 {i+1}/{len(test_wells)}: {wid}...')
    ...
    print(f'  选择器 code={selector_code} variant={selector_variant}')
...
print(f"sub_2 行数: {len(sub_2)}")

print("\n" + "="*50)
print("Step 9: 最终 Blending (0.3*sub_1 + 0.7*sub_2)")
print("="*50)
sub = (
...
print("\n" + "="*50)
print("Step 10: 模型性能对比")
print("="*50)
fold_scores_df = pd.DataFrame(fold_scores)
overall_scores_df = pd.DataFrame({k: [v] for k, v in overall_scores.items()}).T.sort_values(0, ascending=True)
...
axs[0].set_xlabel("")
sns.barplot(x=overall_scores_df.values.flatten(), y=overall_scores_df.index, ax=axs[1], color="grey")
axs[1].set_title("Overall RMSE")
axs[1].set_xlabel("")
axs[1].set_xlim(left=overall_scores_df.values.min() - 0.1, right=overall_scores_df.values.max() + 0.1)
for i, (score, model) in enumerate(zip(overall_scores_df.values.flatten(), overall_scores_df.index)):
    ...
    axs[1].patches[i].set_facecolor(color)
    axs[0].patches[i].set_facecolor(color)
    axs[1].text(score, i, f"{score:.3f}", va="center")
plt.tight_layout()
plt.show()
